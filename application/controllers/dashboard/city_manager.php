...
class City_manager extends CI_Controller
{
	...
	function add_city()
	{
		...
		{	
			...
			$chkduplicate = $this->city_model->getcityByStateCity($state_name,$city_name);
			
			if($chkduplicate){
				$data['msg'] = 'City allready added!';
	...
	function edit_city($id = Null)
	{
		...
		$this->form_validation->set_error_delimiters('<div class="error">', '</div>');
		  ...
		  if($_SERVER['REQUEST_METHOD']=='POST')
		  {
			...
			$chkduplicate = $this->city_model->getcityByStateCity($state_name,$city_name,$id);

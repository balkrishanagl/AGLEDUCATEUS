...
class Ajax_pagination{
    ...
    function create_links(){
        ...
        $CI =& get_instance();    
		//echo $CI->uri->segment($this->uri_segment); die;
        ...
        for ($loop = $start -1; $loop <= $end; $loop++){
            ...
            if ($i >= 0){
                if ($this->cur_page == $loop){
					...
					//echo $loop.$this->cur_tag_close; die;
                    //$output .= $this->cur_tag_open.$loop.$this->cur_tag_close; // Current page
                    $output .= '<li class="active"><a> <span class="page-numbers">'.$loop.'</span></a></li>'; // Current page
                }else{
                    $n = ($i == 0) ? '' : $i;
...
//echo $output; die;
    ...
    function getAJAXlink($count, $text) {
        ...
        return '<a href="javascript:void(0);"' . $this->anchor_class . ' onclick="'.$this->link_func.'('.$pageCount.')" tabindex="0">'. $text .'</a></li>';
    }
}

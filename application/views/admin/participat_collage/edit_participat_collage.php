    ...
    <div class="row">
		...
		<?php if(isset($file_error)){ 
			...
			<?php } ?>
	               ...
	               <form id="add_collage_form" class="form-horizontal" accept-charset="utf-8" method="post" action="<?php echo base_url(); ?>admin/participat_collage/edit_participat_collage/<?php echo $participat_collage_data->id;?>"  enctype="multipart/form-data">          
                        <div class="form-group">
                            <label for="cal_insurance_partner_datetime" class="col-md-4 control-label"><label for="collage title">Title</label></label>
                            <div class="col-md-8">
                                <input type="text" placeholder="Partner Title" class="form-control input-sm br0" size="30" id="collage_title" value="<?php  if(isset($participat_collage_data) && $participat_collage_data != ''){ echo $participat_collage_data->name; }?>" name="collage_title" required/>                            
								<div class="form_er_msg"><?php echo form_error('collage_title'); ?></div>	
                            </div>
                       </div>
							...
							<label for="cal_insurance_partner_info" class="col-md-4 control-label"><label for="main image">Image</label></label>
								<div class="col-md-8">
								   <input type="file"  class="form-control input-sm br0" size="30" id="main_image" name="main_image"/> 
									...
									<?php if(isset($participat_collage_data->main_image) && $participat_collage_data->main_image != ''){ ?>
									<img src="<?php echo base_url().$participat_collage_data->main_image;?>" height="100" width="100"/> 
									   <input type="hidden" value="<?php echo $participat_collage_data->main_image; ?>" name="exist_main_image">
									   <?php } else { ?>

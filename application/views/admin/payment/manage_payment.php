                                ...
                                <?php $i=0; foreach($payment_list as $payment) { $i++;?>
                                    <tr>
									 <td><input type="checkbox" id="pay_id" value="<?php echo $payment->id;?>" name="select_option[]" /></td>
										...
										<?php if($payment->status==1){ echo "Payment Verified"; 
										}else{?> <a onclick="change_status(<?php echo $payment->user_id;?>,<?php echo $payment->id;?>,<?php echo $payment->status; ?>);" href="javascript:void(0);">Active</a> <?php } ?>
      ...
      $('#save_value').change(function(){
        ...
        $(':checkbox:checked').each(function(i){
          val[i] = $(this).val();
		   ...
		   if(val!='') {
			  $.ajax({
            url: "<?php echo base_url(); ?>admin/payment/update_multi_status",
            type: "POST",
...
$(function(){
	$('#btnSearch').click(function(){
		...
		$.ajax({
			...
			url: "<?php echo base_url(); ?>admin/payment/filter_payment",
            type: "POST",
